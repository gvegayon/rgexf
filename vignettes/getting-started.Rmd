...
vignette: >
  %\VignetteIndexEntry{Getting started with rgexf}
...
```{r gexf-js, echo = FALSE, fig.alt = "Les Misérables network rendered with gexf-js"}
...
To embed `gexf-js` in quarto/rmarkdown documents, we can use the `gexfjs` function. This function takes a `gexf` object and returns an HTML widget that can be rendered in the document:

```{r}
gexfjs(graph)
```

Different from the `plot_gexfjs` function, the `gexfjs` inserts the visualization in the document, while `plot_gexfjs` opens it in a new browser window.